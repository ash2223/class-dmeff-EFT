...
//ppt->has_cls == _TRUE_  part is removed
...
last_index=0;
  ...
  /** (b) Compute the non-linear spectrum */
    ...
    if (pnlpt->nonlinear_pt_verbose > 0)
      printf("Computing the non-linear power spectra of matter and biased tracers in perturbation theory.\n");
...
/*old part -- begin    
    ...
    class_alloc(pnlpt->nl_corr_Id2,pnlpt->tau_size*pnlpt->k_size*sizeof(double),pnlpt->error_message);
      ...
      class_alloc(pnlpt->nl_corr_Tree,pnlpt->tau_size*pnlpt->k_size*sizeof(double),pnlpt->error_message);
Old part -- end*/
    ...
    class_alloc(ddlnpk_l,pnlpt->k_size*sizeof(double),pnlpt->error_message);
     ...
     /* Here we convert the requested redshifts into taus */
      ...
      if (myFile22 == NULL){
          printf("Error Reading File M22oneline_N256_packed.dat\n");
          exit (0);
      }
      ...
      if (myFile13 == NULL){
          printf("Error Reading File M13oneline_N256.dat\n");
          exit (0);
      }
      ...
      if (myFile22_basic == NULL){
          printf("Error Reading File M22basiconeline_N256_packed.dat\n");
          exit (0);
      }
      ...
      if (myFile_IFG2 == NULL){
          printf("Error Reading File IFG2oneline_N256.dat\n");
          exit (0);
      }
    ...
    // printf("%i\n",ppr->nmax_nlpt); 
      ...
      for (count=0; count < ppr->nmax_nlpt+1; count++){
          ...
          // printf("%le,%le\n",creal(pnlpt->M13_oneline_complex[count]),cimag(pnlpt->M13_oneline_complex[count]));
      }
...
/* Very crude, but the physics should not depend on these scales anlyway */
...
//    printf("Dfid = %lf\n",Dfid);
    ...
    // Cross-check that the Da and H(z) are computed correctly
...
class_call(background_at_tau(pba,pnlpt->tau[index_tau],long_info,inter_normal,&last_indexD,pvecbackD),pnlpt->error_message,pnlpt->error_message);
    ...
    for (index_k=0; index_k<ppt->k_size[pnlpt->index_md_scalars]; index_k++) {
        ...
        pnlpt->nl_corr_density[index_tau * ppt->k_size[pnlpt->index_md_scalars] + index_k] = sqrt(fabs((pk_Tree_int[index_k]-10000000.+Dplus*Dplus*(pk_nl_int[index_k]-5000.)/Dref/Dref))/(1+pk_l_at_z_req_int[index_k]));
        //printf("pk_Tree_int[index_k]=%f \n",pk_Tree_int[index_k]-10000000);
        //printf("pk_nl_int[index_k]=%f \n",pk_nl_int[index_k]-5000);
        //printf("Dref=%f \n",Dref);
        //printf("pk_l_at_z_req_int[index_k]=%f \n",pk_l_at_z_req_int[index_k]);
      }
    } else {
    for (index_k=0; index_k<ppt->k_size[pnlpt->index_md_scalars]; index_k++) {
        //pnlpt->nl_corr_density[index_tau * ppt->k_size[pnlpt->index_md_scalars] + index_k] = 1.;
        pnlpt->nl_corr_density[index_tau * ppt->k_size[pnlpt->index_md_scalars] + index_k] = sqrt(fabs((pk_Tree[index_k]-10000000.+Dplus*Dplus*(pk_nl[index_k]-5000.)/Dref/Dref))/(1+pk_l_at_z_req[index_k]));
        //printf("pk_Tree[index_k]=%f \n",pk_Tree[index_k]-10000000);
        //printf("pk_nl[index_k]=%f \n",pk_nl[index_k]-5000);
        //printf("Dref=%f \n",Dref);
        //printf("pk_l_at_z_req[index_k]=%f \n",pk_l_at_z_req[index_k]);
      }
    }
    //printf("index_tau=%i \n",index_tau);
    //printf("index_k=%i \n",index_k);
...
// printf(" This module is executed now\n");
...
double kmax = 100. * pba->h;
     /* If you generate new PT matrices, don't forget to choose the kmin and kmax appropriately ! */
...
for (i_kdisc=0; i_kdisc< Nmax; i_kdisc++){
 ...
 //        printf("%le %le\n",kdisc[i],Pdisc[i]);
     ...
     //Introduce function that replaces P with P_regularised for IDM models
     ...
     for (int i=1; i< Nmax-1; i++){
        ...
        double neff = (Pdisc[i+1]-Pdisc[i-1])/(kdisc[i+1]-kdisc[i-1])*kdisc[i]/Pdisc[i];
        if (neff < -3.9 && index_neff == -1){
          index_neff = i;
          n_star = neff;
        }

        if (i > index_neff && index_neff > -1){
          Pdisc[i] = Pdisc[index_neff]*pow(kdisc[i]/kdisc[index_neff],n_star);
        }
        //printf("%le %le\n",kdisc[i],Pdisc[i]);
        //printf("%le %le\n",kdisc[i],(Pdisc[i+1]-Pdisc[i-1])/(kdisc[i+1]-kdisc[i-1])*kdisc[i]/Pdisc[i]);
        
     }

     if (index_neff > -1){
      Pdisc[Nmax-1]=Pdisc[Nmax-2];
     }

     //Regularised function ends here
...
//     kdisc[Nmax - 1] = kdisc[Nmax - 1] -delta_array;
     ...
     /* 
     For my classy.pyx kdiff is -1.563194e-13, and this causes the issue with arrays...
     double kdiff;
     kdiff = kdisc[Nmax-1] - exp(lnk_l[pnlpt->k_size-1]);
     printf("%le\n",kdiff);
      */
    ...
    // printf("sigmav=%le\n",sigmav);
     ...
     /// Check if IR resummation is safe to perform!
     // Check ratio of Pk at 0.01 and 0.3

     last_index = 0;
     double logP001, logP030;
     class_call(array_interpolate_spline(lnk_l,pnlpt->k_size,lnpk_l,myddlnpk,1,log(0.01),&last_index,&logP001,1,pnlpt->error_message),pnlpt->error_message,pnlpt->error_message);
     class_call(array_interpolate_spline(lnk_l,pnlpt->k_size,lnpk_l,myddlnpk,1,log(0.30),&last_index,&logP030,1,pnlpt->error_message),pnlpt->error_message,pnlpt->error_message);
     //if(exp(logP030)/exp(logP001)>2e-2){
     //    pnlpt->irres == pnlpt->irres;
     //    printf("ratio=%e", exp(logP030)/exp(logP001));
     //}
     //else {
     //    printf("ratio=%e", exp(logP030)/exp(logP001));
     //    printf("# Turning off IR resummation for numerical stability!\n");
     ...
     double *Pdisc2;
     class_alloc(Pdisc2,Nir * sizeof(double),pnlpt->error_message);
     double logPbin2;
     double kbin2;
     double *kdisc2;
     class_alloc(kdisc2,Nir * sizeof(double),pnlpt->error_message);
     ...
     for (index_ir=0; index_ir< Nir; index_ir++){
       ...
       Pdisc2[index_ir] = exp(logkPdiscr[index_ir])/kbin2;
     ...
     //Here we implement the regularization procedure for IDM
       ...
       for (int i=1; i< Nir-1; i++){
        ...
        double neff = (Pdisc2[i+1]-Pdisc2[i-1])/(kdisc2[i+1]-kdisc2[i-1])*kdisc2[i]/Pdisc2[i];
        if (neff < -3.9 && index_neff2 == -1){
          index_neff2 = i;
          n_star2 = neff;
        }

        if (i > index_neff2 && index_neff2 > -1){
          Pdisc2[i] = Pdisc2[index_neff2]*pow(kdisc2[i]/kdisc2[index_neff2],n_star2);
        }
        //printf("%le %le\n",kdisc2[i],Pdisc2[i]);
        //printf("%le %le\n",kdisc[i],(Pdisc[i+1]-Pdisc[i-1])/(kdisc[i+1]-kdisc[i-1])*kdisc[i]/Pdisc[i]);
        
        input_realv2[2*i+1] = pow(-1.,1.*i)*log(kdisc2[i]*Pdisc2[i]);
        input_realv2[Nirby4-2*i-1] = pow(-1.,1.*i)*log(kdisc2[i]*Pdisc2[i]);

       }

       if (index_neff2 > -1){
        Pdisc2[Nir-1]=Pdisc2[Nir-2];
        input_realv2[2*(Nir-1)+1] = pow(-1.,1.*(Nir-1))*log(kdisc2[Nir-1]*Pdisc2[Nir-1]);
        input_realv2[Nirby4-2*(Nir-1)-1] = pow(-1.,1.*(Nir-1))*log(kdisc2[Nir-1]*Pdisc2[Nir-1]);
       }

       //printf("%le %le\n",kdisc2[i],Pdisc2[i]);

     // Regularisation ends here
 ...
 //        FFT(input_real_test,input_imag_test,output_real_test,output_imag_test,Nirby4,stepsize);
  ...
  // printf("deltaSigma_BAO(ks=0.2 h/Mpc)=%lf (Mpc/h)^2\n",deltaSigmaBAO * pow(pba->h,2.));
         ...
         // done for the excersise purposes. REMOVE! 
          // SigmaBAO = 0.;
          // deltaSigmaBAO = 0.;
       // SigmaBAO = SigmaBAO/4.;
     ...
     //   IR-5) Computing the LO IR resummed power spectrum
         ...
         // printf("%le %le\n",kdisc[index_kdisc],Pnw[index_kdisc]);
     }
...
// printf("All good class in loop 2.0\n");
...
// free(test2);
   ...
   //  if (pnlpt->rsd == rsd_yes && pnlpt->rsd_only == rsd_only_no || pnlpt->rsd == rsd_no) {
     ...
     // printf("All good class in loop 2.1\n");
...
//         printf("%le %le\n",kdisc[j],P22[j]);
     ...
     for (index_k=0; index_k < pnlpt->k_size; index_k++){
         ...
         class_call(array_interpolate_spline(kdisc,
             ...
             pk_CTR[index_k] = pk_CTR_out + 10000000.;
	         pk_Tree[index_k] = pk_Tree_out + 10000000.;
         }
          ...
          if (pnlpt->k[index_k]>kmax){
             ...
             pk_CTR[index_k] = exp(lnpk_l[index_k]+2.*lnk_l[index_k]) + 10000000.;
             pk_Tree[index_k] = exp(lnpk_l[index_k]) + 10000000.;
         }
...
if (irindex == 1){
   ...
   //     printf("Until now all good\n");
    ...
    double P1loopvv_fog=0.;
    double P1loopvd_fog=0.;
    double P1loopdd_fog=0.;
...
//    for (index_j=0; index_j < Nmax; index_j++){
        ...
        LegendreP4 = (35.*pow(mu,4.)-30.*pow(mu,2.)+3.)/8.;
            ...
            p_tree_vv = (Pnw_ap_out + (1. + Sigmatot*pow(ktrue,2.))* Pw_ap_out * Exp)*pow(f*pow(mutrue,2.),2.)*pnlpt->gauss_w[index_gauss2]*FOGexp*hratio/Dratio/Dratio;
            p_tree_vd = (Pnw_ap_out + (1. + Sigmatot*pow(ktrue,2.))* Pw_ap_out * Exp)*2.*f*pow(mutrue,2.)*pnlpt->gauss_w[index_gauss2]*FOGexp*hratio/Dratio/Dratio;
            p_tree_dd = (Pnw_ap_out + (1. + Sigmatot*pow(ktrue,2.))* Pw_ap_out * Exp)*pnlpt->gauss_w[index_gauss2]*FOGexp*hratio/Dratio/Dratio;
            
            Pctr0 = pow(ktrue,2.)*(Pnw_ap_out+Pw_ap_out*Exp)*pnlpt->gauss_w[index_gauss2]*hratio/Dratio/Dratio;
  //          Pctr2 = (Pnw_ap_out+Pw_ap_out*Exp)*pnlpt->gauss_w[index_gauss2]*pow(b1+f*mutrue*mutrue,2.)*pow(f*mutrue*ktrue,2.)*hratio/Dratio/Dratio;
            ...
            P1loopvv = (p_tree*0. +(P13_mu4_vv_ap_out*P13ratio  + P22_mu4_vv_ap_out + (P22_mu4_vv_w_ap_out+P13_mu4_vv_w_ap_out)*Exp)*pow(mutrue,4.) + (P13_mu6_ap_out*P13ratio + P22_mu6_vv_ap_out+ (P22_mu6_vv_w_ap_out+P13_mu6_w_ap_out)*Exp)*pow(mutrue,6.)+(P22_mu8_ap_out+P22_mu8_w_ap_out*Exp)*pow(mutrue,8.))*pnlpt->gauss_w[index_gauss2]*hratio/Dratio/Dratio*FOGexp;
            
            P1loopdd = ((p_tree*0. + P22_mu0_dd_ap_out + P13_mu0_dd_ap_out*P13ratio + (P13_mu0_dd_w_ap_out+P22_mu0_dd_w_ap_out)*Exp)+(P22_mu2_dd_ap_out + P13_mu2_dd_ap_out*P13ratio + (P22_mu2_dd_w_ap_out+P13_mu2_dd_w_ap_out)*Exp)*pow(mutrue,2.)+(P22_mu4_dd_ap_out+P22_mu4_dd_w_ap_out*Exp)*pow(mutrue,4.))*pnlpt->gauss_w[index_gauss2]*hratio/Dratio/Dratio*FOGexp;
            
            P1loopvd = ((p_tree*2.*f*0. + P13_mu2_vd_ap_out*P13ratio + P22_mu2_vd_ap_out+ (P22_mu2_vd_w_ap_out+P13_mu2_vd_w_ap_out)*Exp)*pow(mutrue,2.)+(P13_mu4_vd_ap_out*P13ratio + P22_mu4_vd_ap_out + (P22_mu4_vd_w_ap_out+P13_mu4_vd_w_ap_out)*Exp)*pow(mutrue,4.)+(P22_mu6_vd_ap_out+P22_mu6_vd_w_ap_out*Exp)*pow(mutrue,6.))*pnlpt->gauss_w[index_gauss2]*hratio/Dratio/Dratio*FOGexp;
            
            P1loopdd_fog = ((p_tree+ P22_mu0_dd_ap_out + P13_mu0_dd_ap_out*P13ratio + (P13_mu0_dd_w_ap_out+P22_mu0_dd_w_ap_out)*Exp)+(P22_mu2_dd_ap_out + P13_mu2_dd_ap_out*P13ratio + (P22_mu2_dd_w_ap_out+P13_mu2_dd_w_ap_out)*Exp)*pow(mutrue,2.)+(P22_mu4_dd_ap_out+P22_mu4_dd_w_ap_out*Exp)*pow(mutrue,4.))*pnlpt->gauss_w[index_gauss2]*hratio/Dratio/Dratio*FOGexp;
            
            P1loopvd_fog = ((p_tree*2.*f + P13_mu2_vd_ap_out*P13ratio + P22_mu2_vd_ap_out+ (P22_mu2_vd_w_ap_out+P13_mu2_vd_w_ap_out)*Exp)*pow(mutrue,2.)+(P13_mu4_vd_ap_out*P13ratio + P22_mu4_vd_ap_out + (P22_mu4_vd_w_ap_out+P13_mu4_vd_w_ap_out)*Exp)*pow(mutrue,4.)+(P22_mu6_vd_ap_out+P22_mu6_vd_w_ap_out*Exp)*pow(mutrue,6.))*pnlpt->gauss_w[index_gauss2]*hratio/Dratio/Dratio*FOGexp;
        ...
        P1loop_2_dd[index_j] +=  P1loopdd_fog*LegendreP2*2.5;
        P1loop_4_dd[index_j] +=  P1loopdd_fog*LegendreP4*4.5;

        P1loop_0_vd[index_j] +=  P1loopvd*LegendreP0/2.;
        P1loop_2_vd[index_j] +=  P1loopvd*LegendreP2*2.5;
        P1loop_4_vd[index_j] +=  P1loopvd_fog*LegendreP4*4.5;
...
// Constructing the final output spectra
    ...
    for (index_k=0; index_k < pnlpt->k_size; index_k++){
        ...
        if (pnlpt->k[index_k]<=kmaxnew && pnlpt->k[index_k]>= kminnew){
            ...
            pk_CTR_0[index_k] = pk_nl_out + 10000000.;
        }
        else{
            pk_CTR_0[index_k] = exp(lnpk_l[index_k]+2.*lnk_l[index_k]) + 10000000.;
        }
    }
    ...
    for (index_k=0; index_k < pnlpt->k_size; index_k++){
        if (pnlpt->k[index_k]<=kmaxnew && pnlpt->k[index_k]>= kminnew){
            class_call(array_interpolate_spline(kdisc,
                       ...
                       pnlpt->error_message,
                       pnlpt->error_message);
            pk_CTR_2[index_k] = pk_nl_out + 10000000.;
        }
        else{
            pk_CTR_2[index_k] = exp(lnpk_l[index_k]+2.*lnk_l[index_k])*f*2./3. + 10000000.;
    ...
    for (index_k=0; index_k < pnlpt->k_size; index_k++){
        if (pnlpt->k[index_k]<=kmaxnew && pnlpt->k[index_k]>= kminnew){
            class_call(array_interpolate_spline(kdisc,
                       ...
                       pnlpt->error_message,
                       pnlpt->error_message);
            pk_CTR_4[index_k] = pk_nl_out + 10000000.;
        }
        
        else{
            pk_CTR_4[index_k] = exp(lnpk_l[index_k]+2.*lnk_l[index_k])*f*f*8./35. + 10000000.;
        }
    }
...
//         printf("f13_IFG2_real=%le f13_IFG2_imag=%le\n",creal(f13_IFG2[index_j]),cimag(f13_IFG2[index_j]));
        ...
        for (index_k=0; index_k < pnlpt->k_size; index_k++){
            ...
            class_call(array_interpolate_spline(kdisc,
                       ...
                       pnlpt->error_message,
                       pnlpt->error_message);
            pk_IFG2[index_k] = pk_IFG2_out + 10000000.;
              }
              else {
               pk_IFG2[index_k] = epsilon_for_logs + 10000000.;
              }
        }
...
if (pnlpt->rsd == rsd_yes) {
 ...
 //           P1 = (Pnw[index_j]+Pw[index_j]*Exp)*f*pow(pnlpt->gauss_x[index_gauss2],2.)*pnlpt->gauss_w[index_gauss2];
            ...
            Pd2d2_in = P_Id2d2_ap_out*pnlpt->gauss_w[index_gauss2]*hratio/Dratio/Dratio * FOGexp2;
            Pd2G2_in = P_Id2G2_ap_out*pnlpt->gauss_w[index_gauss2]*hratio/Dratio/Dratio * FOGexp2;
            PG2G2_in = P_IG2G2_ap_out*pnlpt->gauss_w[index_gauss2]*hratio/Dratio/Dratio * FOGexp2;
            Pb1b2_in = (P_0_b1b2_ap_out + LegendreP2true*P_2_b1b2_ap_out)*pnlpt->gauss_w[index_gauss2]*hratio/Dratio/Dratio * FOGexp2;
            Pb1bG2_in = (P_0_b1bG2_ap_out + LegendreP2true*P_2_b1bG2_ap_out)*pnlpt->gauss_w[index_gauss2]*hratio/Dratio/Dratio * FOGexp2;
            Pb2_in = (P_0_b2_ap_out + LegendreP2true*P_2_b2_ap_out + LegendreP4true*P_4_b2_ap_out)*pnlpt->gauss_w[index_gauss2]*hratio/Dratio/Dratio *FOGexp2;
            PbG2_in = (P_0_bG2_ap_out + LegendreP2true*P_2_bG2_ap_out + LegendreP4true*P_4_bG2_ap_out)*pnlpt->gauss_w[index_gauss2]*hratio/Dratio/Dratio * FOGexp2;
            /*
...
// NEW NEW NEW
/*These functions were copy-pasted from the old spectra module*/
...
int spectra_pk_nl_at_k_and_z_i(
  ...
  class_alloc(spectrum_at_z,pnlpt->ln_k_size*sizeof(double),pnlpt->error_message);
...
// printf("Until here it's Ok\n");
  ...
  class_alloc(spline,sizeof(double)*ppm->ic_ic_size[index_md]*pnlpt->ln_k_size,pnlpt->error_message);
    
    class_alloc(spline_Id2d2,
                sizeof(double)*ppm->ic_ic_size[index_md]*pnlpt->ln_k_size,
                pnlpt->error_message);
