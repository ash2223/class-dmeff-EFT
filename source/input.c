...
int input_read_from_file(struct file_content * pfc,
  ...
  class_call(input_read_precisions(pfc,ppr,pba,pth,ppt,ptr,ppm,phr,pfo,pnlpt,ple,psd,pop,
                                   errmsg),
             errmsg,
             errmsg);
  ...
  /** Find out if shooting necessary and, eventually, shoot and initialize
      read parameters */
  class_call(input_shooting(pfc,ppr,pba,pth,ppt,ptr,ppm,phr,pfo,pnlpt,ple,psd,pop,
                            input_verbose,
                            &has_shooting,
                            errmsg),
             errmsg,
  ...
  *has_shooting=_FALSE_;
  ...
  if (unknown_parameters_size > 0) {
    ...
    *has_shooting=_TRUE_;
    ...
    class_call(input_read_parameters(&(fzw.fc),ppr,pba,pth,ppt,ptr,ppm,phr,pfo,pnlpt,ple,psd,pop,
                                     errmsg),
               errmsg,
               errmsg);
...
int input_get_guess(double *xguess,
  ...
  class_call(input_read_precisions(&(pfzw->fc),&pr,&ba,&th,&pt,&tr,&pm,&hr,&fo,&nlpt,&le,&sd,&op,
                                   errmsg),
             errmsg,
             errmsg);
  ...
  for (index_guess=0; index_guess < pfzw->target_size; index_guess++) {
    ...
    case theta_s:
      ...
      break;
    case Omega_dcdmdr:
      Omega_M = ba.Omega0_cdm+ba.Omega0_dmeff+ba.Omega0_idm_dr+ba.Omega0_dcdmdr+ba.Omega0_b;
      /* *
    ...
    case Omega_ini_dcdm:
      ...
      Omega_M = ba.Omega0_cdm+ba.Omega0_dmeff+ba.Omega0_idm_dr+Omega0_dcdmdr+ba.Omega0_b;
      gamma = ba.Gamma_dcdm/ba.H0;
      if (gamma < 1)
        a_decay = 1.0;
      else
...
int input_try_unknown_parameters(double * unknown_parameter,
  ...
  if (pfzw->required_computation_stage >= cs_nonlinear_pt){
    ...
    class_call_except(nonlinear_pt_init(&pr,&ba,&th,&pt,&pm,&nlpt), nlpt.error_message, errmsg, primordial_free(&pm);perturbations_free(&pt);thermodynamics_free(&th);background_free(&ba));
  }
  ...
  if (pfzw->required_computation_stage >= cs_transfer){
    ...
    class_call_except(transfer_init(&pr,&ba,&th,&pt,&fo,&nlpt,&tr), tr.error_message, errmsg, fourier_free(&fo);primordial_free(&pm);perturbations_free(&pt);thermodynamics_free(&th);background_free(&ba));
  }
  ...
  if (pfzw->required_computation_stage >= cs_spectra){
    ...
    class_call_except(harmonic_init(&pr,&ba,&pt,&pm,&fo,&nlpt,&tr,&hr),hr.error_message, errmsg, transfer_free(&tr);fourier_free(&fo);primordial_free(&pm);perturbations_free(&pt);thermodynamics_free(&th);background_free(&ba));
  }
...
int input_default_params(struct background *pba,
  ...
  pba->Omega0_fld = 0.;
  pba->Omega0_scf = 0.;
  pba->Omega0_lambda = 1.-pba->Omega0_k-pba->Omega0_g-pba->Omega0_ur-pba->Omega0_b-pba->Omega0_cdm-pba->Omega0_dmeff-pba->Omega0_ncdm_tot-pba->Omega0_dcdmdr+pba->Omega0_idr+pba->Omega0_idm_dr;
  /** 8.a) Omega fluid */
  /** 8.a.1) PPF approximation */
  pba->use_ppf = _TRUE_;
