 ...
 * @param pvecback     Input: vector of background quantities (used only in case z>z_initial for getting ddkappa and dddkappa; in that case,
                            should be already allocated and filled, with format normal_info or larger; in other cases, will be ignored)
...
int thermodynamics_init(
  ...
  if (pba->has_dmeff == _TRUE_) {
    ...
    /* fill dTb by splining, use Tdmeff as temporary place holder for 2nd derivative of Tb */
...
int thermodynamics_indices(
  ...
  int index_re;
  /* a running index for the dmeff parameters */
  int index_dmeff;
  ...
  class_define_index(pth->index_ti_Tdm,pba->has_dmeff,index_dmeff,1);
  class_define_index(pth->index_ti_tau,pba->has_dmeff,index_dmeff,1); // must be the last index
  class_define_index(pth->ti_size,pba->has_dmeff,index_dmeff,1);
...
int thermodynamics_dmeff_rate(struct background *pba,
  ...
  //changed this line
  dTb = -pvecthermo[pth->index_th_dTb]*pvecback[pba->index_bg_H]; // dTb/dtau
...
int thermodynamics_dmeff_temperature(struct precision *ppr,
  ...
  //edit 2/16:
  //pvecdmeff_integration[pth->index_ti_Tdm] = 0.;
